...
class App(customtkinter.CTk):
    def _init_(self):
        ...
        # Funcion a maximizar
        self.funcion_max_label = customtkinter.CTkLabel(
            self.frame1, text="Funcion a maximizar")
        self.funcion_max_label.grid(
        ...
        self.x1_entry = customtkinter.CTkEntry(
            self.frame1, width=50, placeholder_text="X1")
        ...
        self.x2_entry = customtkinter.CTkEntry(
            self.frame1, width=50, placeholder_text="X2")
        self.x2_entry.grid(row=1, column=4, padx=10, pady=10)

        # numero de restricciones
        self.num_restr_label = customtkinter.CTkLabel(
            self.frame1, text="Cantidad de Restricciones")
        self.num_restr_label.grid(
            row=2, column=0, columnspan=5, padx=2, pady=5)
        # n restricciones
        self.num_restr_entry = customtkinter.CTkEntry(
            self.frame1, placeholder_text="n")
        self.num_restr_entry.grid(
            row=3, column=0, columnspan=5, padx=2, pady=5)
            ...
            if n_str.isdigit():  # Si el valor de n es un digito
                ...
                for i in range(n):  # Generar todas las entradas de las restricciones
                    restr = customtkinter.CTkLabel(
                        self.frame1, text=f"Restricción {i+1}")
                    restr.grid(row=5+i, column=0, padx=10, pady=5)
                    restr_x1 = customtkinter.CTkEntry(
                        self.frame1, placeholder_text="X1", width=50)
                    restr_x1.grid(row=5+i, column=2, padx=2, pady=5)
                    restr_x2 = customtkinter.CTkEntry(
                        self.frame1, placeholder_text="X2", width=50)
                    restr_x2.grid(row=5+i, column=3, padx=2, pady=5)
                    restr_ind = customtkinter.CTkEntry(
                        self.frame1, placeholder_text="Ind", width=50)
                    restr_ind.grid(row=5+i, column=4, padx=2, pady=5)
                    restrs.append(restr)
                    # agregar una tupla con las tres entradas de la restricción
                    restr_entrys.append((restr_x1, restr_x2, restr_ind))
                generar_restr = customtkinter.CTkButton(
                    self.frame1, text="Calcular", command=graficar)
                generar_restr.grid(row=5+n, column=0,
                                   columnspan=5, padx=10, pady=5)
                restrs.append(generar_restr)
            else:
                CTkMessagebox(
                    title="Error", message="El número de restricciones no es un numero valido", icon="cancel")
        ...
        # Crear el botón
        self.button_restr = customtkinter.CTkButton(
            self.frame1, text="Agregar", command=generar_restricciones)
        self.button_restr.grid(row=4, column=0, columnspan=2, padx=10, pady=5)
        self.button_eliminar = customtkinter.CTkButton(
            self.frame1, text="Eliminar", command=eliminar_etiquetas)
        self.button_eliminar.grid(
            row=4, column=2, columnspan=3, padx=2, pady=5)
        ...
        self.text_area.insert(customtkinter.END, conclusions)
